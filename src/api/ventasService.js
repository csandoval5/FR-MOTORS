 ...
 * usando el auth_id (UUID de Supabase Auth del usuario logueado)
 *
 * @param {string} authId - El UUID del usuario de Supabase Auth (session.user.id)
 * @returns {Promise<number>} El id_usuario numérico
 */
export const obtenerIdUsuario = async (authId) => {
...
export const createVenta = async ({ venta, detalle, session }) => {
  ...
  // usando el auth_id (UUID). Esto evita el error de FK "ventas_id_usuario_fkey".
  let idUsuarioNumerico = venta.id_usuario
  if (session?.user?.id) {
    idUsuarioNumerico = await obtenerIdUsuario(session.user.id)
